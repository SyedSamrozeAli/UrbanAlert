...
async function getUserData(req, res) {
    ...
    try {
        const user = await userModel.findById(id); // finding the user through ID
        console.log("Current User: ", user);

        if (user) {
            return res.send(response(true, 200, "User fetched successully", user)); // Sending success response
        }
